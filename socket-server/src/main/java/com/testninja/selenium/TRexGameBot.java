...
public class TRexGameBot {
    ...
    public void initialize() {
        initializeDriver();
        socketStreamer.initialize(driver);
    }

    public void startGame() {
        socketStreamer.startStreaming();
        initializeGamePage();
        start();
    }
    ...
    private void start() {
        ...
        while (socketStreamer.hasClient()) {
    ...
    private void initializeGamePage() {
        ...
        new WebDriverWait(driver, Duration.ofSeconds(1000)).until(ExpectedConditions.visibilityOf(canvas));
    ...
    public void addClient(SocketIOClient client) {
        socketStreamer.addClient(client);
    }

    public void removeClient(SocketIOClient client) {
        socketStreamer.removeClient(client);
    }
}
